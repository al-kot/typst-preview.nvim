...
---@field pages { total: number, current: number, meta: { width: number, height: number, win_offset: number }[] }
local state = {
    code = {},
    preview = {},
    pages = {
...
local window_height = vim.api.nvim_win_get_height(1000)

function M.render()
    local page_info = state.pages.meta[state.pages.current]
    renderer.render(
        preview_png,
        page_info.win_offset,
        page_info.height,
        page_info.width,
        window_height
    )
...
local function fit_preview_window()
    vim.api.nvim_win_set_width(state.preview.win, state.pages.meta[state.pages.current].width)
    state.pages.meta[state.pages.current].win_offset = vim.fn.win_screenpos(state.preview.win)[2]
end

function M.update_preview_size()
    local cell_width, cell_height = utils.get_cell_dimensions()
    window_height = vim.api.nvim_win_get_height(state.code.win)

    for page = 1, state.pages.total do
        local img_height, img_width = utils.get_page_dimensions(state.code.buf, page)
        ...
        state.pages.meta[page] = { height = rows, width = cols }
    end
